...
let nounThree = "soup"
let sampleStory = "Today was a big day for \(name). They had finally saved up \(number) dollars and were going to buy a \(adjectiveOne) \(nounOne). They went to the \(nounTwo) \(placeOne) feeling very \(positiveEmotion). But then they felt \(negativeEmotion). They were all out of \(nounThree)!"
